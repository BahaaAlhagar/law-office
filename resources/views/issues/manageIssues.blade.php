...
<div id="manageIssues">
		...
		<table v-if="issues.length" class="table table-responsive table-striped table-bordered main-table">
			...
			<tbody>
				<tr v-for="issue in issues" :key="issue.id">
					...
					<td>
						<ul>
							<span style="display: block;" v-for="person in issue.openents">@{{ person.name }} / @{{ openentType(person) }}
							</span>
						</ul>
					</td>
		...
		<br>
		<v-paginator class="heading" v-show="issues.length" :options="options" ref="VP"  :resource_url="resource_url" @update="updateResource"></v-paginator>
		<br>
